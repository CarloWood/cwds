...
// Specifically, this is https://github.com/Eelis/geordi/blob/master/prelude/tracked.hpp
...
/* mute/unmute allow us to suppress boring messages that are not relevant to the issue under consideration.
 * Their use can be a bit tedious though. If we only want to get messages for a few statements inside a statement
 * block, we have to do something like:
...
class focus_t {
 ...
 public:
  ...
  // Mentioning m forces it to exist, but only if the focus_t template is ever instantiated.
  // The effect is that if TRACK (which instantiates focus_t) is ever used, mute() will be called before main() is entered.
  ~focus_t() { mute(); }
  operator bool() const { return false; }
};

template <typename T>
typename focus_t<T>::mute_in_ctor focus_t<T>::m;

} // namespace detail

#define TRACK                                                                                            \
  if (::tracked::detail::focus_t<void> const& tracked_detail_focus = ::tracked::detail::focus_t<void>()) \
    ;                                                                                                    \
  else
...
struct B : protected detail::Tracked {
 ...
 private:
  template <typename C, typename Tr>
  void print(std::basic_ostream<C, Tr>&) const;
};
